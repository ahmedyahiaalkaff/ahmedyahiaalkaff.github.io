...
const Switch = (
  {label, onChange, checked, beforeIcon, afterIcon}
  :
  {
    label?: string,
    onChange: (e:React.ChangeEvent<HTMLInputElement>)=>void,
    checked: boolean
    beforeIcon?: React.JSX.Element,
    afterIcon?: React.JSX.Element,
  }
  ) => {
  return (
    <>
      {beforeIcon && <span className="h-8 w-8">{beforeIcon}</span>}
      <input
        className="mr-3 ml-1 *:mt-[0.3rem] h-3.5 w-8
        ...
        {afterIcon && <span className="h-5 w-5">{afterIcon}</span>}
    </>
  );
}
